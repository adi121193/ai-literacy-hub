...
const researchPapers = [
  {
    ...
    abstract:
      "Comprehensive framework for building AI governance structures that balance innovation with risk management.",
  },
  {
    title: "Implementing ISO 42001: A Practical Playbook",
    abstract:
      "Step-by-step guide to implementing AI management systems aligned with ISO 42001 standards.",
  },
];
          ...
          <div className="space-y-4">
            {recentIssues.map((issue, index) => (
              <a
                key={index}
                href="#"
                className="block p-4 rounded-xl border border-border hover:border-primary transition-smooth hover:shadow-sm"
              >
                ...
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  <div className="flex gap-2">
                    {issue.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded-full bg-accent-light text-primary text-xs"
                      >
                        {tag}
                      </span>
                    ))}
        ...
        <div className="space-y-8 pt-12 border-t border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {researchPapers.map((paper, index) => (
              <div
                key={index}
                className="bg-card rounded-2xl p-8 border border-border hover:border-primary transition-smooth hover:shadow-lg"
              >
                <div className="space-y-4">
